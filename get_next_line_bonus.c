...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/12/12 21:51:32 by tkirihar          #+#    #+#             */
/*   Updated: 2021/12/12 21:51:47 by tkirihar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	line = (char *)malloc(1);
	if (fd < 0 || fd >= FD_MAX || BUFFER_SIZE <= 0 || line == NULL)
		return (free_and_return(line));
	line[0] = '\0';
	check_read = 1;
	if (save[fd] != NULL)
	{
		check_read = join_save_to_line(&line, &save[fd]);
		if (check_read == 1)
			return (line);
		else if (check_read == -1)
			return (free_and_return(line));
	}
	return (read_file(fd, line, &save[fd], check_read));
}
